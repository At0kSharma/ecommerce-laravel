...
<div class="header">
    <!--top nav bar-->
    <div>
        <nav class="navbar navbar-expand-lg navbar-dark color-second-bg shadow-sm p-0 pt-3">
            <div class="container d-flex justify-content-between">
                <!-- Left Side Of Navbar -->
                <ul class="d-flex m-0 p-0 align-items-center">
                    <img src="{{asset('img/logo_gt.png')}}" style="width:3rem;" alt="">
                    <a class="navbar-brand p-0" href="{{ url('/') }}">GurkhaTrails</a>
                </ul>
                ...
                <ul class="d-flex m-0 p-0 list-unstyled align-items-center">
                    ...
                    @guest
                        @if (Route::has('login'))
                            <li class="nav-item">
                                <a class="nav-link text-light pl-0 pr-1" href="{{ route('login') }}">{{ __('Login') }}</a>
                            </li>
                        @endif

                        @if (Route::has('register'))
                            <li class="nav-item">
                                <a class="nav-link text-light p-1" href="{{ route('register') }}">{{ __('Register') }}</a>
                            </li>
                        @endif
                    @else
                        <li class="nav-item dropdown">
                            ...
                            @endif  class="nav-link text-light pl-0 pr-1">
                                {{ Auth::user()->name }}
                            </a>
                        </li>
                        <li>
                            <a class="nav-link text-light p-1" href="{{ route('logout') }}"
                    ...
                    @endguest
                        <li class="nav-item dropdown"> 
                        @if (Cart::count())
                            <a href="{{route('cart.index')}}" class="btn p-1"><i class="fas fa-shopping-bag fa-lg text-white"> <span class="badge badge-light">{{Cart::count()}}</span></i></a>
                        @else
                        <a href="{{route('cart.index')}}" class="nav-link text-light p-1"><i class="fas fa-shopping-bag fa-lg"></i> </a>    
                        @endif
                        </li>
        ...
        <!--second navbar-->
        <nav class="navbar navbar-expand-md navbar-dark color-second-bg shadow-sm pr-5 pl-5">
